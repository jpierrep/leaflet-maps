...
function distribuirArregloEnPartesIguales(arregloOriginal, numeroDePartes) {
  const longitudParte = Math.ceil(arregloOriginal.length / numeroDePartes);
  const partesDistribuidas = [];

  let indiceInicial = 0;
  for (let i = 0; i < numeroDePartes; i++) {
    if (indiceInicial >= arregloOriginal.length) {
      partesDistribuidas.push([]);
    } else {
      const parte = arregloOriginal.slice(indiceInicial, indiceInicial + longitudParte);
      partesDistribuidas.push(parte);
      indiceInicial += longitudParte;
    }
  }

  return partesDistribuidas;
}
...
async function getMatrices(idMatriz,parameter){
  ...
  // Salida: [[1, 2, 3], [4, 5, 6], [7, 8, 9]]
    ...
    console.log("paramenter",parameter)
     ...
     const partesDistribuidas = distribuirArregloEnPartesIguales(mapasNCPendientes, cantPantallas);
     console.log(partesDistribuidas);
  // Salida: [[1, 2, 3], [4, 5, 6], [7, 8, 9]]
